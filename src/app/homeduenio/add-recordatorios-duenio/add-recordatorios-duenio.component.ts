...
import { ConsultaService } from '../../homevet/services/consulta.service';

@Component({
  selector: 'app-add-recordatorios-duenio',
  templateUrl: './add-recordatorios-duenio.component.html',
  styleUrl: './add-recordatorios-duenio.component.css'
})
export class AddRecordatoriosDuenioComponent {
  ...
  constructor(
    public dialogRef: MatDialogRef<AddRecordatoriosDuenioComponent>,
  ...
  onSave(): void {
    if (this.recordatorioForm.valid) {
      ...
      const recordatorioData: RecordatorioDuenioResponse = { ...rest, time };
      this.consultaService.addRecordatorio(recordatorioData).subscribe(
        (response: RecordatorioDuenioResponse) => {
